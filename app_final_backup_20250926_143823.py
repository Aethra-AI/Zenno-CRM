    ...
    try:
        ...
        if user_role == 'Administrador':
            sql = """
                ...
                JOIN Afiliados a ON p.id_afiliado = a.id_afiliado AND a.tenant_id = %s AND a.tenant_id = %s 
...
def get_kpi_reports():
    ...
    try:
        ...
        cursor.execute("""
            ...
            JOIN Postulaciones p ON e.id_postulacion = p.id_postulacion AND p.tenant_id = %s
            JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s AND v.tenant_id = %s
        """)
...
def get_reports():
    ...
    try:
        ...
        if report_name == 'vacantes_activas':
            ...
            cursor.execute("SELECT a.nombre_completo, v.cargo_solicitado, p.fecha_aplicacion FROM Postulaciones p JOIN Afiliados a ON p.id_afiliado = a.id_afiliado AND a.tenant_id = %s AND a.tenant_id = %s JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s AND v.tenant_id = %s ORDER BY p.fecha_aplicacion DESC LIMIT 10")
            results = cursor.fetchall()
        elif report_name == 'entrevistas_agendadas':
            cursor.execute("SELECT e.fecha_hora, a.nombre_completo, v.cargo_solicitado FROM Entrevistas e JOIN Postulaciones p ON e.id_postulacion = p.id_postulacion AND p.tenant_id = %s JOIN Afiliados a ON p.id_afiliado = a.id_afiliado AND a.tenant_id = %s AND a.tenant_id = %s JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s AND v.tenant_id = %s WHERE DATE(e.fecha_hora) >= CURDATE() AND e.tenant_id = %s LIMIT 10", (tenant_id,))
            results = cursor.fetchall()
        else:
        ...
        if request.method == 'GET':
            ...
            try:
                ...
                cursor.execute("""
                    ...
                    JOIN Afiliados a ON p.id_afiliado = a.id_afiliado AND a.tenant_id = %s AND a.tenant_id = %s JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s AND v.tenant_id = %s JOIN Clientes c ON v.tenant_id = c.tenant_id
...
def calendar_activities():
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM Postulaciones p
            LEFT JOIN Afiliados a ON p.id_afiliado = a.id_afiliado AND a.tenant_id = %s AND a.tenant_id = %s
            LEFT JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s AND v.tenant_id = %s
