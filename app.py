...
import re
import hashlib
from celery_tasks import calculate_candidate_score
...
# ===== ENDPOINTS DE ENLACES DE AFILIADO =====

@app.route('/api/users/<int:user_id>/affiliate-link', methods=['POST'])
@token_required
def create_affiliate_link(user_id):
    """
    Crea un enlace de afiliado único para un usuario
    """
    try:
        tenant_id = get_current_tenant_id()
        current_user_id = g.current_user.get('user_id')
        
        # Validar que el usuario solo puede crear su propio enlace o ser admin
        if current_user_id != user_id and not is_admin(current_user_id, tenant_id):
            return jsonify({
                'error': 'No tienes permisos para crear este enlace',
                'code': 'FORBIDDEN'
            }), 403
        ...
        cursor.execute("""
            ...
            WHERE id = %s AND tenant_id = %s AND activo = TRUE
        """, (user_id, tenant_id))
        user = cursor.fetchone()
        if not user:
            return jsonify({'error': 'Usuario no encontrado'}), 404
        ...
        codigo_referencia = ''.join(secrets.choice(characters) for _ in range(code_length))
        
        # Verificar que el código no existe
        max_attempts = 10
        for _ in range(max_attempts):
            cursor.execute("SELECT id FROM TrackingEnlaces WHERE codigo_referencia = %s", (codigo_referencia,))
            if not cursor.fetchone():
                break
            codigo_referencia = ''.join(secrets.choice(characters) for _ in range(code_length))
        
        # Crear entrada en TrackingEnlaces
        github_pages_url = os.getenv('GITHUB_PAGES_URL', 'https://henmir-hn.github.io/portal-empleo-henmir')
        full_url = f"{github_pages_url}?ref={codigo_referencia}"
        
        cursor.execute("""
            INSERT INTO TrackingEnlaces (
                tenant_id, user_id, codigo_referencia, enlace_usado, 
                estado, fecha_click
            ) VALUES (%s, %s, %s, %s, 'click', NOW())
        """, (tenant_id, user_id, codigo_referencia, full_url))
        
        conn.commit()
        cursor.close()
        conn.close()
        
        app.logger.info(f"🔗 Enlace de afiliado creado para usuario {user_id}: {codigo_referencia}")
        
        return jsonify({
            'success': True,
            'codigo_referencia': codigo_referencia,
            'url': full_url,
            'message': 'Enlace de afiliado creado exitosamente'
        }), 201
        
    except Exception as e:
        app.logger.error(f"Error creando enlace de afiliado: {str(e)}")
        return jsonify({'error': 'Error al crear enlace de afiliado'}), 500


@app.route('/api/users/<int:user_id>/affiliate-links', methods=['GET'])
@token_required
def get_affiliate_links(user_id):
    ...
    try:
        ...
        if current_user_id != user_id and not is_admin(current_user_id, tenant_id):
            return jsonify({
                'error': 'No tienes permisos para ver estos enlaces',
                'code': 'FORBIDDEN'
            }), 403
        ...
        cursor.execute("""
            SELECT 
                ...
                fecha_click,
                fecha_registro,
                (SELECT COUNT(*) FROM TrackingEnlaces t2 
                 WHERE t2.codigo_referencia = TrackingEnlaces.codigo_referencia 
                 AND t2.estado = 'registrado') as total_registrados
        ...
        app.logger.error(f"Error obteniendo enlaces de afiliado: {str(e)}")
        return jsonify({'error': 'Error al obtener enlaces de afiliado'}), 500
...
def upload_multiple_cvs_to_oci():
    ...
    try:
        ...
        # Guardar información del job en la tabla CV_Processing_Logs
        conn = get_db_connection()
        if not conn:
            return jsonify({'error': 'Error de conexión a la base de datos'}), 500
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO CV_Processing_Logs (
                    ...
                    %s, %s, 'batch_start', 'processing', 
                    %s, %s, NOW()
                )
            """, (
                tenant_id, 
                job_id, 
                f'Iniciando procesamiento de {len(valid_files)} archivos',
                json.dumps({
                    'total_files': len(valid_files),
                    'processed_files': 0,
                    'successful_files': 0,
            ...
            def process_files_background():
                try:
                    app.logger.info(f"🔄 INICIANDO PROCESAMIENTO EN SEGUNDO PLANO para job {job_id}")
                    ...
                    cursor.execute("""
                        UPDATE CV_Processing_Logs 
                        SET processing_step = 'batch_completed', status = 'completed', created_at = NOW()
                        WHERE tenant_id = %s AND cv_identifier = %s
                    """, (tenant_id, job_id))
                    conn.commit()
                    ...
                    try:
                        cursor.execute("""
                            UPDATE CV_Processing_Logs 
                            SET processing_step = 'batch_failed', status = 'failed', message = %s, created_at = NOW()
                            WHERE tenant_id = %s AND cv_identifier = %s
                        """, (str(batch_error), tenant_id, job_id))
                        conn.commit()
                    except:
                        pass
            ...
            import threading
            app.logger.info(f"🚀 Iniciando thread para procesamiento de {len(valid_files)} archivos")
            thread = threading.Thread(target=process_files_background)
            thread.daemon = True
            thread.start()
            app.logger.info(f"✅ Thread iniciado exitosamente")
...
@token_required
def get_cv_processing_status(job_id):
    """
    ...
    try:
        ...
        cursor.execute("""
            SELECT 
                cv_identifier, processing_step, status, message, details, created_at
        ...
        if details.get('total_files', 0) > 0:
            progress_percent = (details.get('processed_files', 0) / details.get('total_files', 1)) * 100
        
        response_data = {
            'success': True,
            'job_data': {
                'job_id': job_data['cv_identifier'],
                'total_files': details.get('total_files', 0),
                'processed_files': details.get('processed_files', 0),
                'successful_files': details.get('successful_files', 0),
                'failed_files': details.get('failed_files', 0),
...
@token_required
def get_cv_batch_results(job_id):
    """
    Obtener resultados detallados del procesamiento masivo de CVs
    """
    try:
        tenant_id = get_current_tenant_id()
        ...
        response_data = {
            'success': True,
            'summary': {
                'successful': details.get('successful_files', 0),
                'duplicates': 0,  # No aplica para CVs
                'errors': details.get('failed_files', 0),
                'total': details.get('total_files', 0)
            },
            'results': [],  # Se puede expandir para incluir detalles específicos
            'job_id': job_id,
            'status': job_data['status'],
            'message': job_data['message']
        }
        ...
        app.logger.error(f"Error obteniendo resultados del job {job_id}: {str(e)}")
...
def create_candidate_from_web_form(manual_data, cv_data, tenant_id, cv_url, referrer_user_id=None, codigo_referencia=None):
    ...
    Args:
        ...
        referrer_user_id: ID del usuario que generó el enlace de afiliado (opcional)
    ...
    try:
        ...
        if existing_candidate:
            ...
            if referrer_user_id:
                app.logger.info(f"📌 Candidato será asociado al usuario {referrer_user_id} vía TrackingEnlaces")
            ...
            if referrer_user_id and codigo_referencia:
                try:
                    ...
                    cursor.execute("""
                        INSERT INTO TrackingEnlaces (
                            tenant_id, user_id, candidato_id, codigo_referencia, 
                            enlace_usado, ip_address, user_agent, 
                    ...
                    """, (
                        tenant_id, referrer_user_id, candidate_id, codigo_referencia,
                        'Registro desde web', ip_address, user_agent
                    ))
                    conn.commit()
                    app.logger.info(f"✅ Registrado en TrackingEnlaces: candidato {candidate_id} asociado a usuario {referrer_user_id}")
                except Exception as e:
                    app.logger.warning(f"Error registrando tracking: {str(e)}")
                    conn.rollback()
...
def register_candidate_from_web():
    ...
    try:
        ...
        # Si hay código de referencia, buscar el usuario que lo generó
        if codigo_referencia:
            try:
                ...
                if tracking and tracking['tenant_id'] == tenant_id:
                    referrer_user_id = tracking['user_id']
                    app.logger.info(f"📌 Candidato registrado con código de referencia: {codigo_referencia} del usuario {referrer_user_id}")
                cursor.close()
                conn.close()
            except Exception as e:
                app.logger.warning(f"Error validando código de referencia: {str(e)}")
                # Continuar sin atribución
...
# --- PUBLIC ENDPOINTS FOR CLAWDBOT AGENT ---
@app.route('/api/agents/deploy', methods=['POST'])
@token_required
def deploy_tenant_agent_endpoint():
    """Endpoint para que un Admin o Tenant active su propio agente Clawdbot"""
    tenant_id = get_current_tenant_id()
    user_data = g.current_user
    user_id = user_data.get('user_id')

    # Solo permitimos si es admin del tenant o superadmin
    if not is_admin(user_id, tenant_id):
        return jsonify({"success": False, "message": "No tienes permisos para activar el agente"}), 403

    conn = get_db_connection()
    try:
        # 1. Obtener o crear API Key para el bot si no existe
        api_keys = public_api_service.get_api_keys_by_tenant(conn, tenant_id)
        bot_key = next((k for k in api_keys if k['nombre_descriptivo'] == 'ESC_BOT_KEY'), None)
        
        if not bot_key:
            res = public_api_service.create_api_key(
                conn, tenant_id, "ESC_BOT_KEY", "Key automatica para el agente Clawdbot", user_id
            )
            if not res['success']: return jsonify(res), 500
            api_key_str = res['api_key']
        else:
            # Si ya existe, necesitamos la original (en este diseño asumimos que el orquestador la conoce 
            # o la generamos/rotamos para el despliegue)
            api_key_str = bot_key['api_key']

        # 2. Ejecutar despliegue vía Orquestador
        # Usamos variables de entorno para la URL del CRM y la Master Key del LLM
        crm_internal_url = os.getenv('ESC_CRM_INTERNAL_URL', 'http://esc-backend:5000')
        master_llm_key = os.getenv('OPENAI_API_KEY') 
        ...
        if success:
            return jsonify({
                "success": True, 
                "message": "Agente desplegado e iniciando...",
                "container": info
            }), 200
        else:
...
def proxy_agent_chat():
    """Proxy para enviar mensajes del chat web al agente del tenant"""
    tenant_id = get_current_tenant_id()
    data = request.get_json()
    ...
    # La URL del agente es su nombre de contenedor dentro de la red docker
    agent_url = f"http://esc-agent-tenant-{tenant_id}:18791/chat"
    
    try:
        # Reenviamos la petición al agente aislado
        # Usamos stream=True para peticiones largas si fuera necesario
        response = requests.post(agent_url, json={"message": message}, timeout=60)
        return jsonify(response.json()), response.status_code
    except Exception as e:
        app.logger.error(f"Chat proxy error: {str(e)}")
        return jsonify({"error": "El agente no responde. Asegúrate de haberlo activado e intenta de nuevo."}), 503

@app.route('/api/agents/status', methods=['GET'])
@token_required
def get_agent_status():
    """Verifica si el agente del tenant está activo"""
    tenant_id = get_current_tenant_id()
