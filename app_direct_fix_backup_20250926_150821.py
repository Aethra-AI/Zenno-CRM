...
def login():
    ...
    try:
        ...
        app.logger.error(f"Error en función: {str(e)}")
...
def get_reports():
    ...
    try:
        ...
        if report_name == 'vacantes_activas':
            ...
            cursor.execute("SELECT a.nombre_completo, v.cargo_solicitado, p.fecha_aplicacion FROM Postulaciones p JOIN Afiliados a ON p.id_afiliado = a.id_afiliado AND a.tenant_id = %s AND a.tenant_id = %s AND a.tenant_id = %s JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s AND v.tenant_id = %s AND v.tenant_id = %s ORDER BY p.fecha_aplicacion DESC LIMIT 10")
            results = cursor.fetchall()
        elif report_name == 'entrevistas_agendadas':
            cursor.execute("SELECT e.fecha_hora, a.nombre_completo, v.cargo_solicitado FROM Entrevistas e JOIN Postulaciones p ON e.id_postulacion = p.id_postulacion AND p.tenant_id = %s AND p.tenant_id = %s JOIN Afiliados a ON p.id_afiliado = a.id_afiliado AND a.tenant_id = %s AND a.tenant_id = %s AND a.tenant_id = %s JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s AND v.tenant_id = %s AND v.tenant_id = %s WHERE DATE(e.fecha_hora) >= CURDATE() AND e.tenant_id = %s LIMIT 10", (tenant_id,))
            results = cursor.fetchall()
        else:
...
def get_candidates():
    ...
    try:
        ...
        count_query = query.replace("SELECT 
                ...
                (SELECT GROUP_CONCAT(DISTINCT c.empresa SEPARATOR ', ') 
                 FROM Postulaciones p 
                 JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s AND v.tenant_id = %s
                 JOIN Clientes c ON v.tenant_id = c.tenant_id 
