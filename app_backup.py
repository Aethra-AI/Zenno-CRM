...
# Decorador para manejar CORS preflight
def cors_preflight():
...
# Manejar CORS preflight para todas las rutas
@app.before_request
...
def login():
    ...
    try:
        ...
        # Obtener el conteo de sesiones por separado para evitar problemas de subconsultas
        sessions_count = 0
        if user:
            cursor.execute("""
                SELECT COUNT(*) as count FROM UserSessions 
                WHERE user_id = %s AND expira > NOW()
            """, (user['id'],))
            sessions_result = cursor.fetchone()
            sessions_count = sessions_result['count'] if sessions_result else 0
...
def get_current_user():
    ...
    try:
        ...
        if user:
            ...
            cliente_nombre = cliente_result['empresa'] if cliente_result else ""
        if not user:
...
def get_vacancy_details(vacancy_name: str):
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "SELECT cargo_solicitado, empresa FROM Vacantes v JOIN Clientes c ON v.tenant_id = c.tenant_id WHERE v.cargo_solicitado LIKE %s AND v.tenant_id = %s LIMIT 1"
...
def postulate_candidate_to_vacancy(vacancy_id: int, candidate_id: int = None, identity_number: str = None, comments: str = ""):
    ...
    try:
        ...
        tenant_id = get_current_tenant_id()
        cursor.execute("SELECT id_postulacion FROM Postulaciones WHERE id_afiliado = %s AND id_vacante = %s AND tenant_id = %s", (final_candidate_id, vacancy_id, tenant_id))
        if cursor.fetchone():
        ...
        sql = "INSERT INTO Postulaciones (id_afiliado, id_vacante, fecha_aplicacion, estado, comentarios, tenant_id) VALUES (%s, %s, NOW(), 'Recibida', %s)"
...
def prepare_whatsapp_campaign_tool(message_body: str, candidate_id: int = None, identity_number: str = None, candidate_ids: str = None, vacancy_id: int = None, application_date: str = None):
    ...
    try:
        ...
        if not final_message_body:
            ...
            tenant_id = get_current_tenant_id()
            cursor.execute("SELECT cuerpo_mensaje FROM Whatsapp_Templates WHERE id_template = 1 AND tenant_id = %s", (tenant_id,))
        ...
        if candidate_id or identity_number:
            ...
            if target_id:
                tenant_id = get_current_tenant_id()
                cursor.execute("SELECT id_afiliado, nombre_completo, telefono FROM Afiliados WHERE id_afiliado = %s AND tenant_id = %s", (target_id, tenant_id))
        ...
        # Prioridad 2: Si se da una lista de IDs/identidades.
            ...
            if id_list:
                ...
                sql = f"SELECT id_afiliado, nombre_completo, telefono FROM Afiliados WHERE (id_afiliado IN ({placeholders}) OR identidad IN ({placeholders})) AND tenant_id = %s"
...
def schedule_interview_tool(postulation_id: int, interview_date: str, interview_time: str, interviewer: str, notes: str = ""):
    ...
    try:
        ...
        sql_insert = "INSERT INTO Entrevistas (id_postulacion, fecha_hora, entrevistador, resultado, observaciones, tenant_id) VALUES (%s, %s, %s, 'Programada', %s)"
        ...
        if report_name == 'vacantes_activas':
            sql = """
                SELECT v.cargo_solicitado, c.empresa, 
                       (SELECT COUNT(*) FROM Postulaciones p WHERE p.id_vacante = v.id_vacante AND p.tenant_id = %s) as total_postulaciones
...
def search_vacancies_tool(city: str = None, keyword: str = None):
    ...
    try:
        ...
        query = "SELECT cargo_solicitado, ciudad FROM Vacantes WHERE estado = 'Abierta' AND tenant_id = %s"
        params = []
    ...
    try:
        ...
        sql = """
            ...
            FROM Postulaciones p 
                JOIN Afiliados a ON p.id_afiliado = a.id_afiliado AND a.tenant_id = %s AND a.tenant_id = %s AND a.tenant_id = %s 
...
def update_application_status(id_postulacion):
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM Postulaciones p
            JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s AND v.tenant_id = %s AND v.tenant_id = %s
...
def get_kpi_reports():
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) as total_candidates FROM Afiliados WHERE tenant_id = %s", (tenant_id,))
        total_candidates = cursor.fetchone()['total_candidates']
        
        cursor.execute("SELECT COUNT(*) as new_this_month FROM Afiliados WHERE MONTH(fecha_registro) = MONTH(CURDATE()) AND YEAR(fecha_registro) = YEAR(CURDATE()) AND tenant_id = %s", (tenant_id,))
        ...
        cursor.execute("""
            ...
            JOIN Postulaciones p ON e.id_postulacion = p.id_postulacion AND p.tenant_id = %s AND p.tenant_id = %s
            JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s AND v.tenant_id = %s AND v.tenant_id = %s
        """)
...
def get_reports():
    ...
    try:
        ...
        if report_name == 'vacantes_activas':
            tenant_id = get_current_tenant_id()
            cursor.execute("SELECT id_vacante, cargo_solicitado, fecha_apertura FROM Vacantes WHERE estado = 'Activa' AND tenant_id = %s LIMIT 10", (tenant_id,))
        results = cursor.fetchall()
        elif report_name == 'postulaciones_recientes':
            tenant_id = get_current_tenant_id()
            cursor.execute("SELECT a.nombre_completo, v.cargo_solicitado, p.fecha_aplicacion FROM Postulaciones p JOIN Afiliados a ON p.id_afiliado = a.id_afiliado AND a.tenant_id = %s JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s WHERE p.tenant_id = %s ORDER BY p.fecha_aplicacion DESC LIMIT 10", (tenant_id, tenant_id, tenant_id))
            results = cursor.fetchall()
        elif report_name == 'entrevistas_agendadas':
            tenant_id = get_current_tenant_id()
            cursor.execute("SELECT e.fecha_hora, a.nombre_completo, v.cargo_solicitado FROM Entrevistas e JOIN Postulaciones p ON e.id_postulacion = p.id_postulacion AND p.tenant_id = %s JOIN Afiliados a ON p.id_afiliado = a.id_afiliado AND a.tenant_id = %s JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s WHERE DATE(e.fecha_hora) >= CURDATE() AND e.tenant_id = %s LIMIT 10", (tenant_id, tenant_id, tenant_id, tenant_id))
            results = cursor.fetchall()
        else:
...
def get_dashboard_data():
    ...
    try:
        # Métricas básicas
        tenant_id = get_current_tenant_id()
        cursor.execute("SELECT COUNT(*) as total FROM Entrevistas WHERE fecha_hora >= CURDATE() AND tenant_id = %s", (tenant_id,))
        entrevistas_pendientes = cursor.fetchone()['total']
        cursor.execute("SELECT COUNT(*) as total FROM Entrevistas WHERE fecha_hora < CURDATE() AND resultado = 'Programada' AND tenant_id = %s", (tenant_id,))
        entrevistas_sin_resultado = cursor.fetchone()['total']
        
        # Estadísticas de vacantes
        cursor.execute("SELECT V.cargo_solicitado, C.empresa, COUNT(P.id_postulacion) as postulantes FROM Postulaciones P JOIN Vacantes V ON P.id_vacante = V.id_vacante AND V.tenant_id = %s JOIN Clientes c ON v.tenant_id = c.tenant_id AND c.tenant_id = %s WHERE P.tenant_id = %s GROUP BY V.id_vacante, V.cargo_solicitado, C.empresa ORDER BY postulantes DESC", (tenant_id, tenant_id, tenant_id))
        ...
        cursor.execute("SELECT COUNT(*) as total FROM Afiliados WHERE MONTH(fecha_registro) = MONTH(CURDATE()) AND YEAR(fecha_registro) = YEAR(CURDATE()) AND tenant_id = %s", (tenant_id,))
        afiliados_mes = cursor.fetchone()['total']
        
        # Top ciudades
        cursor.execute("SELECT ciudad, COUNT(*) as total FROM Afiliados WHERE ciudad IS NOT NULL AND ciudad != '' AND tenant_id = %s GROUP BY ciudad ORDER BY total DESC LIMIT 5", (tenant_id,))
...
def get_dashboard_metrics():
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE p.tenant_id = %s AND c.tenant_id = %s
        """, (tenant_id, tenant_id))
        ...
        cursor.execute("""
            ...
            FROM Contratados c
            JOIN Postulaciones p ON c.id_afiliado = p.id_afiliado AND c.id_vacante = p.id_vacante
            WHERE c.fecha_contratacion IS NOT NULL AND p.fecha_aplicacion IS NOT NULL AND c.tenant_id = %s AND p.tenant_id = %s
        """, (tenant_id, tenant_id))
        ...
        cursor.execute("""
            SELECT 
                ...
                COUNT(*) as total
            FROM Afiliados 
            WHERE fecha_registro >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH) AND tenant_id = %s
        ...
        cursor.execute("""
            ...
            FROM Contratados
            WHERE fecha_contratacion >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH) AND tenant_id = %s
        """, (tenant_id,))
        ...
        cursor.execute("""
            ...
            FROM Clientes c
            LEFT JOIN Vacantes v ON c.tenant_id = v.tenant_id AND v.tenant_id = %s
        ...
        cursor.execute("""
            ...
            WHERE p.tenant_id = %s AND co.tenant_id = %s
        """, (tenant_id, tenant_id))
    ...
    try:
        ...
        query = """
            SELECT 
                ...
                a.skills,
                a.comentarios,
                (SELECT COUNT(*) FROM Postulaciones p WHERE p.id_afiliado = a.id_afiliado AND p.tenant_id = %s) as total_aplicaciones,
        ...
        count_query = query.replace("""SELECT 
                ...
                (SELECT GROUP_CONCAT(DISTINCT c.empresa SEPARATOR ', ') 
                 FROM Postulaciones p 
                 JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s
                 JOIN Clientes c ON v.tenant_id = c.tenant_id 
                 WHERE p.id_afiliado = a.id_afiliado AND p.tenant_id = %s) as empresas_aplicadas""", "SELECT COUNT(*) as total")
...
def update_candidate_score(candidate_id):
    ...
    try:
        ...
        user_id = g.current_user.get('id')  # Obtener el ID del usuario autenticado
...
def handle_applications():
    ...
    try:
        if request.method == 'GET':
            ...
            cursor.execute("SELECT id_postulacion FROM Postulaciones WHERE id_afiliado = %s AND id_vacante = %s AND tenant_id = %s", (data['id_afiliado'], data['id_vacante'], tenant_id))
        ...
        if request.method == 'GET':
            ...
            try:
                ...
                cursor.execute("""
                    ...
                    JOIN Afiliados a ON p.id_afiliado = a.id_afiliado AND a.tenant_id = %s AND a.tenant_id = %s AND a.tenant_id = %s JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s AND v.tenant_id = %s AND v.tenant_id = %s JOIN Clientes c ON v.tenant_id = c.tenant_id
...
def handle_clients():
    ...
    cursor = conn.cursor(dictionary=True)
        ...
        if request.method == 'GET':
            ...
            data = request.get_json()
            sql = "INSERT INTO Clientes (empresa, contacto_nombre, telefono, email, sector, observaciones, tenant_id) VALUES (%s, %s, %s, %s, %s, %s)"
...
def create_user(user_data):
    ...
    try:
        ...
        tenant_id = get_current_tenant_id()
        cursor.execute("SELECT id FROM Users WHERE email = %s AND tenant_id = %s", (user_data['email'], tenant_id))
        if cursor.fetchone():
...
def update_user(user_id, user_data):
    ...
    try:
        ...
        if 'email' in user_data:
            cursor.execute("SELECT id FROM Users WHERE email = %s AND id != %s AND tenant_id = %s", 
...
def create_user_route():
    ...
    try:
        ...
        cursor = conn.cursor()
        tenant_id = get_current_tenant_id()
        cursor.execute("SELECT id FROM Users WHERE email = %s AND tenant_id = %s", (email, tenant_id))
        if cursor.fetchone():
            return jsonify({
...
def bot_validate_registration():
    ...
    try:
        ...
        query = "SELECT id_afiliado, nombre_completo FROM Afiliados WHERE identidad = %s AND tenant_id = %s LIMIT 1"
        cursor.execute(query, (clean_identity,))
        result = cursor.fetchone()
...
def bot_get_all_active_vacancies():
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "SELECT cargo_solicitado FROM Vacantes WHERE estado = 'Abierta' AND tenant_id = %s ORDER BY cargo_solicitado ASC"
...
def bot_get_vacancy_details():
    ...
    try:
        ...
        query = "SELECT cargo_solicitado, requisitos FROM Vacantes WHERE estado = 'Abierta' AND LOWER(cargo_solicitado) LIKE LOWER(%s) AND tenant_id = %s LIMIT 1"
        params = (f"%{cargo}%",)
...
def bot_get_candidate_status():
    ...
    try:
        ...
        tenant_id = get_current_tenant_id()
        cursor.execute("SELECT id_afiliado, nombre_completo FROM Afiliados WHERE identidad = %s AND tenant_id = %s", (clean_identity, tenant_id))
...
def recalculate_stale_scores(self):
    ...
    try:
        ...
        app.logger.info(f"Iniciando recálculo de puntuaciones para {total} candidatos")
        ...
        for i, candidate in enumerate(candidates, 1):
            ...
            try:
                ...
                app.logger.error(f"Error al programar cálculo para candidato {candidate_id}: {str(e)}")
        ...
        app.logger.error(f"Error en recalculate_stale_scores: {str(e)}")
        raise
    finally:
        if 'cursor' in locals():
            cursor.close()
...
def bot_get_vacancies_with_details():
    ...
    cursor = conn.cursor(dictionary=True)
    try:
        query = "SELECT cargo_solicitado, ciudad, requisitos FROM Vacantes WHERE estado = 'Abierta' AND tenant_id = %s"
        cursor.execute(query)
        results = cursor.fetchall()
...
def get_all_affiliates_for_sync():
    ...
    try:
        ...
        query = "SELECT identidad, telefono FROM Afiliados WHERE telefono IS NOT NULL AND telefono != '' AND tenant_id = %s"
        cursor.execute(query)
...
def get_chat_context_by_identity(identity_number):
    ...
    try:
        ...
        tenant_id = get_current_tenant_id()
        cursor.execute("SELECT id_afiliado, nombre_completo, identidad, telefono, ciudad FROM Afiliados WHERE identidad = %s AND tenant_id = %s", (clean_identity, tenant_id))
...
def create_initial_user():
    ...
    try:
        ...
        cursor.execute("SELECT id FROM Users WHERE email = %s AND tenant_id = %s", (initial_email, 1))
        if cursor.fetchone():
        ...
        query = "INSERT INTO Users (email, password_hash, tenant_id) VALUES (%s, %s, %s)"
...
def calendar_activities():
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM Postulaciones p
            LEFT JOIN Afiliados a ON p.id_afiliado = a.id_afiliado AND a.tenant_id = %s AND a.tenant_id = %s AND a.tenant_id = %s
            LEFT JOIN Vacantes v ON p.id_vacante = v.id_vacante AND v.tenant_id = %s AND v.tenant_id = %s AND v.tenant_id = %s
